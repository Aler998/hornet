...
import { useDeleteTripMutation, useGetTripsQuery } from '../../features/trips/tripsApi';
...
export default function Index() {
  ...
  return (
    ...
    <>
      ...
      <DashboardContent>
        {
          error ? <p>Errore nel caricamento dei viaggi</p> : (
            <>
              <Box
                sx={{
                  mb: 5,
              ...
              >
                ...
                <Button
                  onClick={() => navigate(`/${import.meta.env.VITE_SUBFOLDER}/trips/create`)}
                  variant="contained"
                  color="inherit"
                  ...
                  <TableContainer sx={{ overflow: 'unset' }}>
                    <Table sx={{ minWidth: 800 }}>
                      ...
                      <TableBody>
                        {dataFiltered?.slice(
                          ...
                          .map((row) => (
                            <DefaultTableRow
                              ...
                              onShow={() => navigate(`/${import.meta.env.VITE_SUBFOLDER}/trips/${row.slug}/show`)}
                              onEdit={() => navigate(`/${import.meta.env.VITE_SUBFOLDER}/trips/${row.slug}/edit`)}
                              onDelete={() => confirmDelete(row.slug)}
                            >
                              <>
                                <TableCell key={row.slug} component="th" scope="row">
                                  <Box
                                    sx={{
                                      gap: 2,
                                      display: 'flex',
                                ...
                                <TableCell>{row.start ? dayjs(row.start).format("YYYY-MM-DD HH:mm") : ""}</TableCell>
                                <TableCell>{row.end ? dayjs(row.end).format("YYYY-MM-DD HH:mm") : ""}</TableCell>
                                <TableCell>
                                  <Label color={'success'}>{dayjs(row.end).diff(row.start, 'minute', false)} min</Label>
                                </TableCell>
                                <TableCell>
                                  <Label>
                                    {categories?.find(c => c._id === row.category)?.title}
                                  </Label>
                                </TableCell>
                              </>
                            </DefaultTableRow>
                        ...
                        <TableEmptyRows
                          height={68}
                          emptyRows={emptyRows(table.page, table.rowsPerPage, trips ? trips.length : 0)}
                        />
