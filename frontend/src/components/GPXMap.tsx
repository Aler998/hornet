...
const GPXMap = ({ serverData, classes }: { serverData: string, classes?:string }) => {
    ...
    const extractPolylineCoords = (geojson: FeatureCollection<Geometry, GeoJsonProperties>) => {
        ...
        geojson.features.forEach(feature => {
            if (feature.geometry.type === 'LineString') {
                coords.push(feature.geometry.coordinates.map(([lon, lat]) => [lat, lon]));
            }
        });
        return coords;
    };
    ...
    return (
        <MapContainer className={classes} center={[45.4642, 9.19]} zoom={13} style={{ width: '100%' }}>
            <TileLayer
