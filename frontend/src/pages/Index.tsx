...
function Index() {
    ...
    return (
        <Layout template={templates.default} isLoading={isLoading} title='I nostri viaggi'>
            {trips ? (
                <>
                    <Masonry breakpointCols={breakpointColumnsObj}
                        ...
                        {trips.map(trip => {
                            if (trip.images.length > 0) {
                                return (
                                    <div key={trip._id} className="block relative group w-full break-inside-avoid">
                                        <Link to={`/trip/${trip.slug}`} className="block relative w-full">
                                            <img className="w-full h-full" src={import.meta.env.VITE_ASSETS_URL + trip.images[0].path} />
