...
export const fetchAndCombineCSVData = async (
  ...
  // da lanciare in caso non funzioni
  // node -e "require('https').get('https://www.mimit.gov.it', res => console.log(res.statusCode)).on('error', console.error)"
  ...
  return await axios
    .get(anagraficaUrl, { timeout: 60000 })
    .then(async (anagrafica) => {
        ...
        for (const row of prezziRows) {
          ...
          if (columns.length >= 4) {
            ...
            if (result[key]) {
              ...
              if (
                !existing ||
                parseFloat(prezzo) < parseFloat(existing.prezzo)
              ) {
...
export const checkAndFetchFile = async () => {
  if (
